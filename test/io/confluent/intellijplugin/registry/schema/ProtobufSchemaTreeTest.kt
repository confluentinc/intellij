...
class ProtobufSchemaTreeTest {
    ...
    private fun buildTreeFor(fixture: String): Pair<DefaultMutableTreeNode, ProtobufSchemaTree> {
    ...
    fun `scalar fields are added with lowercased proto type names`() {
        val (root, _) = buildTreeFor("complex.proto")
    ...
    fun `enum field exposes all enum value names as children`() {
        ...
        assertTrue(values.containsAll(listOf("NEW", "PAID")))
    }

    @Test
    fun `nested message field is added as a placeholder until expanded`() {
        val (root, tree) = buildTreeFor("complex.proto")
