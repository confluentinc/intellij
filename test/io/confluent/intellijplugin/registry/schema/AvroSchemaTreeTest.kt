...
class AvroSchemaTreeTest {
    ...
    private fun buildTreeFor(fixture: String): Triple<DefaultMutableTreeNode, AvroSchemaTree, JTree> {
    ...
    inner class `when building tree` {
        ...
        fun `record fields are added with their schema type names`() {
            val (root, _, _) = buildTreeFor("nested.avsc")
        ...
        @Test
        fun `nested record is added as a placeholder leaf`() {
            val (root, _, _) = buildTreeFor("nested.avsc")
        ...
        fun `non-record root is added as a single named node`() {
            val (root, _, _) = buildTreeFor("non-record.avsc")
    ...
    inner class `when expanding nodes` {
        ...
        fun `treeExpanded replaces placeholder with record fields`() {
            val (root, tree, jTree) = buildTreeFor("nested.avsc")
