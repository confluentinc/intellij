...
class ConnectionDataTest {
    ...
    companion object {
        ...
        fun <T> Class<T>.superclassesAndInterfaces(): Set<Class<*>> {
            return generateSequence<Set<Class<*>>>(setOf(this)) { classes ->
                classes.flatMap { it.interfaces.toList() + it.superclass }.filterNotNull().toSet().minus(classes)
                    .takeIf { it.isNotEmpty() }
            }.drop(1).reduce(Set<Class<*>>::plus)
        }

        fun <T> DeclaredProperty<T>.overriding(): List<DeclaredProperty<out Any>> {
            return c.superclassesAndInterfaces().flatMap { superclass -> superclass.declaredProperties() }
                .filter { it.p.name == p.name }
        }

        fun <T : ConnectionData> DeclaredProperty<T>.isOverrideMentioned(): Boolean {
            val superClasses = c.superclasses().drop(1)
            val overriding =
                superClasses.flatMap { superclass -> superclass.declaredProperties() }.filter { it.p.name == p.name }
    ...
    fun connectionDataFields() {
        ...
        val kindToNames =
            nameToKind.groupBy { it.second }.mapValues { it.value.map(Pair<String, String>::first) }.toMutableMap()
