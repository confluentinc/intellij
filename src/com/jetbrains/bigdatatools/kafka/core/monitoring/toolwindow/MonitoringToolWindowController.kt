...
import com.jetbrains.bigdatatools.kafka.core.ui.AutorefreshPopupComponent
...
abstract class MonitoringToolWindowController(protected val project: Project) : Disposable {
  ...
  open fun setUp(toolWindow: ToolWindow) {
    ...
    ApplicationManager.getApplication().messageBus.connect(this).subscribe(LafManagerListener.TOPIC, LafManagerListener {
      contentManager.contents.forEach {
        ...
        val dataManager = (DriverManager.getDriverById(project, connectionId) as? MonitoringDriver)?.dataManager ?: return@forEach
  ...
  private fun setupActions(connectionId: String?) {
    ...
    (toolWindow as? ToolWindowEx)?.setTitleActions(listOfNotNull(
    ...
    DriverManager.onDriversInit(project) {
      invokeLater {
        if (connectionData.isEnabled &&
            RfsConnectionDataManager.instance?.getConnections(project)?.any { it.innerId == connectionData.innerId } == true) {
          val mainController = createMainController(connectionData)
          val content = contentManager.factory.createContent(mainController.getComponent(), connectionData.name, false).apply {
  ...
  private fun createEmptyContent(project: Project): Content {
    ...
    val panel = JBPanelWithEmptyText().apply {
      emptyText.apply {
        ...
        appendSecondaryText(KafkaMessagesBundle.message("toolwindow.empty.link"), SimpleTextAttributes.LINK_ATTRIBUTES, settingsOpener)
        appendSecondaryText(" " + KafkaMessagesBundle.message("toolwindow.empty.secondaryText"), StatusText.DEFAULT_ATTRIBUTES, null)
  ...
  private fun createDisabledContent(project: Project, connectionData: ConnectionData): Content {
    ...
    val panel = JBPanelWithEmptyText().apply {
      emptyText.apply {
        appendText(KafkaMessagesBundle.message("services.panel.connection.disabled", connectionData.name), StatusText.DEFAULT_ATTRIBUTES)
        appendSecondaryText(KafkaMessagesBundle.message("depend.connection.action.enable"), SimpleTextAttributes.LINK_ATTRIBUTES, settingsOpener)
        isShowAboveCenter = false
      }
    }
  ...
  inner class MonitoringConnectionListener : ConnectionSettingsListener {
    ...
    override fun onConnectionModified(project: Project?, connectionData: ConnectionData, modified: Collection<ModificationKey>) {
      ...
      val contentToInsert = contentManager.contents.firstOrNull { it.getUserData(CONNECTION_ID) == connectionData.innerId }
  ...
  inner class MonitoringSettingsAction : DumbAwareAction(KafkaMessagesBundle.message("open.settings.action.text"),
  ...
  inner class CreateNewConnectionAction : DumbAwareAction(KafkaMessagesBundle.message("toolwindow.tab.add.title"),
