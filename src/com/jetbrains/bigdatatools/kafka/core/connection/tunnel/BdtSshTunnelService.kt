...
object BdtSshTunnelService {
  ...
  private val createdTunnels = ConcurrentHashMap<Pair<ConnectionSshTunnelInfo, TunnelHandler.CallerId>, TunnelHandler>()
   ...
   */
  fun createIfRequired(project: Project?,
                       tunnelData: ConnectionSshTunnelData,
                       uri: String,
                       connectionId: String,
                       isTest: Boolean): UriTunnelHandler? {
    if (!tunnelData.isEnabled) return null

    if (uri.contains(",")) {
      throw BdtConfigurationException(KafkaMessagesBundle.message("connection.error.tunnel.should.be.created.just.for.one.url", uri))
    }
    val url = try {
    ...
    val connectionSshTunnelInfo = ConnectionSshTunnelInfo(tunnelData.configId, remoteHost = url?.host ?: "", remotePort = url?.port ?: 0,
   ...
   * Disposing of result object leads to destroying the tunnel only is there are no other (distinct) object pointing to the same tunnel.
   */
  fun createIfRequiredInternal(project: Project?,
                               connectionSshTunnelInfo: ConnectionSshTunnelInfo?,
                               connectionId: String,
                               isTest: Boolean): TunnelHandler? {
    ...
    return createdTunnels.compute(connectionSshTunnelInfo to callerId) { _, v ->
      if (v == null || v == existingTunnel) {
        TunnelHandler(localPort, connectionSshTunnelInfo, callerId, session, portForwardingListMember).also { tunnelHandler ->
...
class TunnelHandler internal constructor(val localPort: Int,
                                         val tunnelInfo: ConnectionSshTunnelInfo,
                                         val callerId: CallerId,
                                         val session: SshSession,
                                         private val portForwardingListMember: String) : Disposable {
  ...
  fun forUri(originalUri: String): UriTunnelHandler {
    ...
    val tunnelledUrl = URL(urlObject.protocol, NetUtils.getLocalHostString(), localPort, urlObject.file,
                           BdtUrlUtils.getUrlHandler(BdtUrlUtils.getProtocol(urlObject.toExternalForm()))).toExternalForm()
    return UriTunnelHandler(tunnelledUrl, originalUri, this)
  }
}

class UriTunnelHandler(val tunnelledUri: String,
                       @Suppress("unused") val originalUri: String,
                       tunnelHandler: TunnelHandler) : Disposable by tunnelHandler
