...
object KafkaDialogFactory {
  fun createTopicDialog(uiDisposable: Disposable, dataManager: KafkaDataManager) {
    val builder = DialogBuilder()
    builder.addOkAction()
    builder.addCancelAction()
    builder.title(KafkaMessagesBundle.message("action.create.topic"))
    val nameField = JTextField()
    val numPartition = JTextField()
    ...
    val validator = object : InputValidatorEx {
      ...
      override fun getErrorText(inputString: String) = if (inputString.isBlank())
    ...
    val centralPanel = MigPanel().apply {
      ...
      row(KafkaMessagesBundle.message("dialog.create.topic.num.partition"), numPartition)
    }
