...
class KafkaSettingsCollector : BdtSettingsCollector() {
  ...
  init {
    ...
    registryStringEnumEvent(KafkaSettingsCustomizer::registryType, KafkaRegistryType.values().map { it.id })
    registryStringEnumEvent(KafkaSettingsCustomizer::brokerConfSource, KafkaConfigurationSource.values().map { it.id })
    registryStringEnumEvent(KafkaSettingsCustomizer::brokerCloudSource, KafkaCloudType.values().map { it.id })
    registryStringEnumEvent(KafkaSettingsCustomizer::brokerPropertiesSource, KafkaPropertySource.values().map { it.id })
    ...
    registryStringEnumEvent(KafkaSettingsCustomizer::brokerAuthType, KafkaAuthMethod.values().map { it.id })
    ...
    registryStringEnumEvent(KafkaSettingsCustomizer::brokerSaslMechanism, KafkaSaslMechanism.values().map { it.name })
