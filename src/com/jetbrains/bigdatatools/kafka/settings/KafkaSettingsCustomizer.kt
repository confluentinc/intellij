...
class KafkaSettingsCustomizer(project: Project, connectionData: KafkaConnectionData, private val uiDisposable: Disposable) :
  ...
  override fun getDefaultFields(): List<WrappedComponent<in KafkaConnectionData>> {
    val wrappedComponents: List<WrappedComponent<in KafkaConnectionData>> = (listOf(nameField, url,
                                                                                    tunnelField)
                                                                             + brokerSettings.getDefaultFields()
                                                                             + registrySettings.getDefaultFields()
                                                                            ) as List<WrappedComponent<in KafkaConnectionData>>
