...
class AwsSettingsComponentForKafka(val includeRegionSetting: Boolean = false, val onChanged: (AwsSettingsComponentForKafka) -> Unit) {
  private val authTypes = listOf(AuthenticationType.DEFAULT, AuthenticationType.PROFILE_FROM_CREDENTIALS_FILE, AuthenticationType.KEY_PAIR)
      ...
      row {
        link(KafkaMessagesBundle.message("open.credentials")) {
          try {
            ...
            val credFile = defaultCredentialFile.takeIf { it.exists() } ?: AwsConnectionUtils.createCredentialFileIfDoesNotExists()
                           ?: return@link
  ...
  fun getInfo(): StaticAwsSettingsInfo = when (authTypeChooser.component.item) {
    ...
    AuthenticationType.PROFILE_FROM_CREDENTIALS_FILE -> StaticAwsSettingsInfo(authenticationType = authTypeChooser.component.item.id,
                                                                              profile = profileComboBox.component.item,
                                                                              region = getRegionId())
    AuthenticationType.KEY_PAIR -> StaticAwsSettingsInfo(authenticationType = authTypeChooser.component.item.id,
