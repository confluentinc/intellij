        ...
        return when (depth) {
            ...
            4 -> {
                if (rfsPath.name == TOPICS_FOLDER) {
                    ...
                    // Get data plane cache for this cluster and fetch topics
                    ...
                    topics.map { topic ->
                        ConfluentFileInfo(this, topicPath(envId, clusterId, topic.topicName))
                    }
                    ...
                    // Get data plane cache for this cluster and fetch subjects
                    val cache = dataManager.getDataPlaneCache(cluster)

                    if (!cache.hasSchemaRegistry()) {
                        log.warn("ConfluentDriver: No Schema Registry available for environment $envId")
                        return emptyList()
                    }
                    ...
                    subjects.map { subject ->
                        ConfluentFileInfo(this, schemaPath(envId, srId, subject.name))
                    }
                } else {
