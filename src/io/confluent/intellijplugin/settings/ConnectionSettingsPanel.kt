...
class ConnectionSettingsPanel(val project: Project) : MasterDetailsComponent(), ConfigurableUi<RfsConnectionDataManager>, ConnectionSettingsListener {
  ...
  fun getTreeNode(innerId: String): MyNode? {
    return TreeUtil.treeNodeTraverser(tree.model.root as DefaultMutableTreeNode).filter(MyNode::class.java).find { node ->
  ...
  override fun createActions(fromPopup: Boolean): List<AnAction> {
    ...
    val addAction = RootAddActionGroup(KafkaMessagesBundle.message("settings.addConnection.text"),
                                       IconUtil.addIcon,
                                       KafkaMessagesBundle.message("settings.addConnection.hint")).apply {
      registerCustomShortcutSet(CommonActionsPanel.getCommonShortcut(CommonActionsPanel.Buttons.ADD), tree)
    }

    return listOf(addAction,
  ...
  private fun createTreeNode(group: ConnectionGroup,
                             data: ConnectionData,
  ...
  fun createNewConnectionFor(group: ConnectionFactory<*>, data: ConnectionData? = null, selectAddedNode: Boolean = true): ConnectionData {
    ...
    parentToGroups.keySet().forEach { parentId ->
      if (parentId != null) {
        parentToGroups.get(parentId).forEach {
          ...
          val tlgActionNode = topLevelGroups.find { topLevelActionNode -> topLevelActionNode.group.id == tlg.id }!!
  ...
  private fun treeWalkUpRemove(fromNode: MyNode) {
    ...
    while (current != null && current.parent != myRoot && current.parent?.childCount == 1) {
      ...
      (current.configurable as? GroupEmptyConfigurable)?.group?.id?.let { groupToActionNode[it] }?.removeFromTree()
  ...
  private abstract inner class AbstractAddActionGroup(@Nls(capitalization = Nls.Capitalization.Title) text: String?,
                                                      icon: Icon?,
  ...
  private inner class RootAddActionGroup(@Nls(capitalization = Nls.Capitalization.Title) text: String? = null,
                                         icon: Icon? = null,
  ...
  private inner class NodeAddActionGroup(val children: Collection<ActionNode>,
  ...
  private inner class DuplicateConnectionAction : DumbAwareAction(KafkaMessagesBundle.message("settings.duplicateConnection"), null,
      ...
      (myNode.configurable as? ConnectionConfigurable<*, *>)?.let {
        ...
        runWithModalProgressBlocking(project, KafkaMessagesBundle.message("progress.title.credentials.saving")) {
          ...
          connectionData.credentialIds().toHashSet().forEach { credentialsId ->
            connectionData.getCredentials(credentialsId)?.let { credentials -> newData.setCredentials(credentials, credentialsId) }
          }
        }
  ...
  private class FakeTitledBorder(keywords: Collection<String>) : TitledBorder(BorderFactory.createEmptyBorder(),
