...
class ColumnVisibilitySettings(var visibleColumns: MutableList<String>) {
    ...
    private open class ConfigColumnsVisibilityAction(
        columns: List<*>,
        settings: ColumnVisibilitySettings
    ) : SmartPopupActionGroup(), DumbAware {
        init {
            ...
            for (field in columns) {
                add(object : DumbAwareToggleAction(
                    field.localizedColumn(),
                    KafkaMessagesBundle.message("show.hide.column"),
                    null
                ) {
                    override fun isSelected(e: AnActionEvent) = settings.isColumnVisible(field.name())
                    override fun getActionUpdateThread() = ActionUpdateThread.BGT
                    override fun setSelected(e: AnActionEvent, state: Boolean) =
                        settings.setColumnVisible(field.name(), state)
                })
            }
        }
    ...
    companion object {
        ...
        fun createRightAlignedAction(
            columns: List<LocalizedField<*>>,
            settings: ColumnVisibilitySettings
