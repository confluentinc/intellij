...
//import io.confluent.intellijplugin.core.settings.kerberos.BdtJaasConfig
//import io.confluent.intellijplugin.core.settings.kerberos.KerberosSettingsDialog
...
import io.confluent.intellijplugin.core.util.PathUtils
import io.confluent.intellijplugin.registry.KafkaRegistryType
import io.confluent.intellijplugin.rfs.*
import io.confluent.intellijplugin.rfs.KafkaConnectionData.Companion.CONFIG_KEY
...
internal class KafkaBrokerSettings(
    val project: Project,
    ...
    internal val confSource = RadioGroupField(
        KafkaConnectionData::brokerConfigurationSource,
        KafkaSettingsCustomizer.KafkaSettingsKeys.CONFIGURATION_SOURCE_KEY,
        connectionData,
        KafkaConfigurationSource.entries
    ).apply {
        addItemListener {
    ...
    internal val cloudSource = RadioGroupField(
        KafkaConnectionData::brokerCloudSource,
        KafkaSettingsCustomizer.KafkaSettingsKeys.CLOUD_PROVIDER,
        connectionData,
        KafkaCloudType.entries
    ).apply {
        addItemListener {
    ...
    internal val propertiesSource = RadioGroupField(
    ...
    private val propertiesCredentialsHolder =
        CredentialsHolder(connectionData, CONFIG_KEY, uiDisposable, coroutineScope)
    ...
    internal val propertiesFile = BrowseTextField(
        ...
        FileChooserDescriptorFactory.createSingleFileDescriptor()
            .withTitle(KafkaMessagesBundle.message("settings.properties.file.browse")),
        initSettings = connectionData,
    ).apply {
        withEmptyOrFileExistValidator(uiDisposable, canBeEmpty = false)
    }

    internal val confluentSettings =
        KafkaConfluentSettings(project, connectionData, uiDisposable, url, propertiesEditor, this)
    ...
    internal val authMethod =
        RadioComboBox(KafkaAuthMethod.entries.toTypedArray(), KafkaAuthMethod.NOT_SPECIFIED).apply {
            addItemListener {
            ...
            cloudGroup = rowsRange {
                ...
                awsMskGroup = rowsRange {
                    row {
                        ...
                        mskUrl.text(
                            propertiesEditor.getProperties()?.get(CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG) ?: ""
                        )
                        mskUrl.onChanged {
                        ...
                        contextHelp(
                            KafkaMessagesBundle.message("settings.msk.setup.desc"),
    ...
    private fun Panel.createImplicitSettingsGroup() = rowsRange {
        ...
        saslGroup = indent {
            row(KafkaMessagesBundle.message("kafka.sasl.mechanism")) {
                saslMechanism = comboBox(
                    KafkaSaslMechanism.entries,
            ...
            saslKerberosGroup = indent {
                row {
                    saslKerberosUseTicketCache =
                        checkBox(KafkaMessagesBundle.message("settings.use.kerberos.cache")).onChanged {
                ...
                saslAdditionalKerberosGroup = rowsRange {
                    ...
                    row(KafkaMessagesBundle.message("kerberos.connection.settings.keytab.label")) {
                        saslKeytab = textFieldWithBrowseButton(
                            browseDialogTitle = KafkaMessagesBundle.message("kerberos.connection.settings.keytab.select.dialog.title"),
                            project
                        ) {
    ...
    private fun getKafkaPropertiesFromUi(): Map<String, String?> {
        val result =
            mutableMapOf<String, String?>(CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG to url.getTextComponent().text)
        ...
        when (authMethod.selectedItem) {
            ...
            KafkaAuthMethod.SASL -> {
                ...
                result += mapOf(
                    CommonClientConfigs.SECURITY_PROTOCOL_CONFIG to securityProtocol.name,
    ...
    private fun addSslProperties(result: MutableMap<String, String?>) {
        ...
        result += mapOf(
            SslConfigs.SSL_TRUSTSTORE_LOCATION_CONFIG to config.truststoreLocation.ifBlank { null },
        ...
        if (config.useKeyStore) {
            result += mapOf(
                SslConfigs.SSL_KEYSTORE_LOCATION_CONFIG to config.keystoreLocation.ifBlank { null },
    ...
    private fun setAwsProperties(properties: Map<String, String>, settingsUi: AwsSettingsComponentForKafka) {
        ...
        val bdtJaasConfig = try {
            BdtJaasConfig(jaasConfig).config?.options?.map { it.key.lowercase() to (it.value?.toString() ?: "") }
                ?.toMap() ?: return
        } catch (_: Throwable) {
            return
        }
    ...
    private fun updateVisibilityOfSasl() {
        ...
        saslCredentialsGroup.visible(
            mechanism in setOf(
                KafkaSaslMechanism.PLAIN,
                KafkaSaslMechanism.SCRAM_256,
                KafkaSaslMechanism.SCRAM_512
            )
        )
    ...
    fun getDefaultFields(): List<WrappedComponent<in KafkaConnectionData>> = listOf(
        propertiesEditor, propertiesFile,
        propertiesSource, confSource,
        cloudSource
    ) + confluentSettings.getDefaultFields()
