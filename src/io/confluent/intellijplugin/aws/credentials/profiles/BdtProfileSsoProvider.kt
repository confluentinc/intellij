...
import io.confluent.intellijplugin.aws.credentials.sso.BdtSsoAccessTokenProvider
import io.confluent.intellijplugin.aws.credentials.sso.BdtSsoCredentialProvider
import io.confluent.intellijplugin.aws.credentials.sso.SsoPrompt
import io.confluent.intellijplugin.aws.credentials.sso.SsoSupport
...
class BdtProfileSsoProvider(profile: Profile, profileFile: ProfileFile) : AwsCredentialsProvider, Disposable {
    ...
    init {
        ...
        ssoClient =
            SsoClient.builder().region(Region.of(ssoRegion)).credentialsProvider(AnonymousCredentialsProvider.create())
                .build()
        ssoOidcClient = SsoOidcClient.builder().region(Region.of(ssoRegion))
