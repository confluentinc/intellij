...
import io.confluent.intellijplugin.aws.credentials.promptForMfaToken
import io.confluent.intellijplugin.aws.credentials.utils.StsAuthenticationException
...
class BdtProfileAssumeRoleProvider(
    ...
    init {
        ...
        val roleSessionName = profile.property(
            ProfileProperty.ROLE_SESSION_NAME
        ...
        val durationSecs =
            profile.property(ProfileProperty.DURATION_SECONDS).map { it.toIntOrNull() }.orElse(null) ?: 3600
    ...
    override fun resolveCredentials(): AwsCredentials = try {
        ...
        try {
            ProfileCredentialProviderFactory.instance.clearCache(
                profile.name(),
                configFilePath = null,
