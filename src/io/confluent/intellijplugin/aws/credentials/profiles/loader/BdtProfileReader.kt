...
package io.confluent.intellijplugin.aws.credentials.profiles.loader

import com.intellij.openapi.diagnostic.thisLogger
import io.confluent.intellijplugin.aws.credentials.profiles.BdtCredentialSourceType
import io.confluent.intellijplugin.aws.credentials.profiles.propertyExists
import io.confluent.intellijplugin.aws.credentials.profiles.requiredProperty
import io.confluent.intellijplugin.aws.credentials.profiles.traverseCredentialChain
...
object BdtProfileReader {
        ...
        if (credentials?.exists() == false)
            error(
                KafkaMessagesBundle.message(
                    "connection.error.profile.creds.file.is.not.found",
    ...
    private fun validateProfile(profile: Profile, profileFile: ProfileFile) {
        when {
            ...
            profile.propertyExists(ProfileSection.SSO_SESSION.propertyKeyName) -> validateSsoWithSessionProfile(
                profile,
                profileFile
            )
            ...
            profile.propertyExists(ProfileProperty.CREDENTIAL_PROCESS) -> {
                ...
                throw IllegalArgumentException(
                    KafkaMessagesBundle.message(
                        "credentials.profile.unsupported",
                        profile.name()
                    )
                )
            }
    ...
    private fun validateAssumeRoleProfile(profile: Profile, profileFile: ProfileFile) {
        ...
        if (credentialSource.isPresent) {
            try {
                ...
                throw IllegalArgumentException(
                    KafkaMessagesBundle.message(
                        "credentials.profile.assume_role.invalid_credential_source",
                        rootProfile.name()
                    )
                )
            }
