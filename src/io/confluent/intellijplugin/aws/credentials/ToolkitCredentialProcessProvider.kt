...
class ToolkitCredentialProcessProvider internal constructor(
    ...
    private fun refresh(): RefreshResult<AwsCredentials> {
        ...
        if (output.isTimeout) {
            handleException(
                KafkaMessagesBundle.message(
                    "credentials.profile.credential_process.timeout_exception_prefix",
                    entrypoint
                ), output,
                null
            )
        ...
        if (output.exitCode != 0) {
            handleException(
                KafkaMessagesBundle.message(
                    "credentials.profile.credential_process.execution_exception_prefix",
                    entrypoint
                ), output,
                null
            )
        ...
        val result = try {
            ...
            handleException(
                KafkaMessagesBundle.message("credentials.profile.credential_process.parse_exception_prefix"),
                output,
                e
            )
        }
