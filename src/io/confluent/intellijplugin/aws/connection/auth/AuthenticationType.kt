...
class AuthenticationType private constructor(val id: String, @Nls val title: String) {
    companion object {
        val DEFAULT: AuthenticationType = AuthenticationType(
            "Default credential providers chain",
            KafkaMessagesBundle.message("settings.auth.type.default")
        )
        val KEY_PAIR: AuthenticationType =
            AuthenticationType("Explicit access key and secret key", KafkaMessagesBundle.message("auth.type.keypair"))

        @Deprecated(message = "Use PROFILE_FROM_CREDENTIALS_FILE")
        val NAMED_PROFILE: AuthenticationType =
            AuthenticationType("Named profile", KafkaMessagesBundle.message("auth.type.namedprofile"))
        val PROFILE_FROM_CREDENTIALS_FILE: AuthenticationType = AuthenticationType(
