...
enum class AwsRegionGroup(override val id: String, override val title: String) : RenderableEntity {
  ...
  companion object {
    ...
    private val asiaRegions = listOf(Region.AP_EAST_1, Region.AP_SOUTHEAST_3, Region.AP_SOUTH_1, Region.AP_SOUTH_2, Region.AP_NORTHEAST_3,
    ...
    private val europeRegions = listOf(Region.EU_CENTRAL_1, Region.EU_CENTRAL_2, Region.EU_WEST_1, Region.EU_WEST_2, Region.EU_SOUTH_1,
                                       Region.EU_SOUTH_2, Region.EU_WEST_3,
                                       Region.EU_NORTH_1)

    //Do not show in UI!
    private val specialGlobalRegion = listOf(Region.AWS_GLOBAL, Region.AWS_CN_GLOBAL,
    ...
    private val globalRegions = usRegions + asiaRegions + europeRegions + canadaRegions + southAmerica + africaRegions + middleEast

    init {
      val otherRegions = Region.regions() - globalRegions - usGovRegions - chinaRegions - globalRegions - specialGlobalRegion
    ...
    fun getRegionSubgroupsForGroup(group: AwsRegionGroup?): List<List<Region>> = when (group) {
      GLOBAL -> listOf(usRegions, africaRegions, asiaRegions, canadaRegions, europeRegions, middleEast, southAmerica)
      CHINA -> listOf(chinaRegions)
      US_GOV -> listOf(usGovRegions)
      null -> getRegionSubgroupsForGroup(GLOBAL) + getRegionSubgroupsForGroup(CHINA) + getRegionSubgroupsForGroup(US_GOV)
    }
  }
}