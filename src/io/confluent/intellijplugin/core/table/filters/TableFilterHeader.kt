...
class TableFilterHeader(table: JTable) : JPanel(BorderLayout()), PropertyChangeListener {
    ...
    inner class FilterColumnsControllerPanel(private val table: JTable, font: Font?, foreground: Color?) :
                ...
                while (it.hasNext()) {
                    ...
                    if (fcp.tableColumn === tc) {
                        ...
                        if (previous != null) {
                            ...
                            val overPreviousDragging =
                                if (rightToLeft) header.draggedDistance > 0 else header.draggedDistance < 0
        ...
        fun placeComponents() {
            var x = 0
            val it =
                if (table.componentOrientation == ComponentOrientation.RIGHT_TO_LEFT) columns.descendingIterator() else columns.iterator()
