...
class TableRowFilter(var table: JTable) : RowFilter<TableModel, Int>() {
  ...
  fun setConditions(conditions: List<Pair<Int, String>>) {
    ...
    conditions.forEach {
      ...
      val filter: ((Any?) -> Boolean) = if (complex == null || (cls != Int::class.java && cls != Long::class.java && cls != Double::class.java)) {
        if (cls == Date::class.java) {
          ...
          { value: Any? -> if (value == null) false else DateRenderer.df.format(value).contains(it.second) }
        }
        else {
