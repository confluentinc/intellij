...
open class MaterialTable : JBTable, Disposable, UiDataProvider, CopyProvider {
  ...
  companion object {
    private val cellBorder = BorderFactory.createCompoundBorder(IdeBorderFactory.createBorder(JBColor.LIGHT_GRAY, SideBorder.LEFT),
  ...
  override fun prepareRenderer(renderer: TableCellRenderer, row: Int, column: Int): Component {
    ...
    val (foregroundColor, backgroundColor) = selectCellColors(isRowSelected(row), isColumnSelected(column), tableIsFocusOwner, row, column)
    c.font = font
   ...
   */
  protected open fun selectCellColors(isRowSelected: Boolean,
                                      isColumnSelected: Boolean,
                                      isFocusOwner: Boolean,
                                      row: Int,
                                      column: Int): Pair<Color, Color> {
    return when {
      isRowSelected -> {
        if (isColumnSelected) {
