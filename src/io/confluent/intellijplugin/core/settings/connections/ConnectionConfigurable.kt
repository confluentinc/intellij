...
import io.confluent.intellijplugin.core.settings.ConnectionSettingsPanel
import io.confluent.intellijplugin.core.settings.defaultui.DefaultConnectionSettingsPanel
...
abstract class ConnectionConfigurable<D : ConnectionData, C : SettingsPanelCustomizer<D>>(
    ...
    override fun createOptionsPanel(): DefaultConnectionSettingsPanel<D> {
        coroutineScope =
            SafeExecutor.createInstance(disposable).coroutineScope.plus(ModalityState.any().asContextElement())
        ...
        val panel = DefaultConnectionSettingsPanel(
            project,
            connectionData,
            settingsCustomizer,
            createConnectionTesting(),
            disposable,
            coroutineScope
        )
    ...
    protected open fun createConnectionTesting(): ConnectionTesting<D>? =
        RfsConnectionTestingBase(project, settingsCustomizer)
