...
import io.confluent.intellijplugin.core.rfs.driver.depend.MasterConnectionData
import io.confluent.intellijplugin.core.settings.*
import io.confluent.intellijplugin.core.settings.connections.ConnectionData
import io.confluent.intellijplugin.core.settings.connections.ConnectionData.Companion.TEST_SUFFIX
...
class DefaultConnectionSettingsPanel<D : ConnectionData>(
    ...
    val perProjectCheckbox = JCheckBox(KafkaMessagesBundle.message("connection.per.project"), conn.isPerProject).also {
        it.isEnabled =
            conn.sourceConnection == null && (conn !is MasterConnectionData<*> || conn.getSlaveConnections().isEmpty())
        if (!it.isEnabled)
    ...
    private fun createDefaultFields(): MutableList<WrappedComponent<in D>> {
        ...
        return mutableListOf(
            ...
            StringNamedField(ConnectionData::uri, CommonSettingsKeys.URL_KEY, conn).withNotEmptyValidator(
                validatorDisposable
            ),
            NullableIntNamedField(ConnectionData::port, CommonSettingsKeys.PORT_KEY, conn).withNumberValidator(
                validatorDisposable
            ),
    ...
    init {
            ...
            defaultComponent = settingsCustomizer.getDefaultComponent(wrappedComponents, conn)?.let {
                MigPanel().apply {
                    ...
                    if (masterConnectionId != null) {
                        val connection = RfsConnectionDataManager.instance?.getConnectionById(
                            project = project,
                            connId = masterConnectionId
                        )
        ...
        if (defaultComponent == null) {
            ...
            if (masterConnectionId != null) {
                val connection =
                    RfsConnectionDataManager.instance?.getConnectionById(project = project, connId = masterConnectionId)
                val masterName = connection?.name
                masterName?.let {
                    defaultComponent.row(
                        JLabel(
                            KafkaMessagesBundle.message(
                                "notification.connection.is.managed.by.driver",
                                masterName
                            )
                        )
                    )
    ...
    fun validateConfig() {
        ...
        if (validationErrors.isNotEmpty()) {
            val configName =
                conn.name.ifBlank { KafkaMessagesBundle.message("validator.emptyConfigName", conn.groupId) }

            @Suppress("HardCodedStringLiteral", "DialogTitleCapitalization")
            throw ConfigurationException(
                validationErrors.joinToString(System.lineSeparator()) { it.message },
