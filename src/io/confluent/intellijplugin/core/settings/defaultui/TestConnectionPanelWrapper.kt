...
class TestConnectionPanelWrapper<D : ConnectionData>(private val connectionTesting: ConnectionTesting<D>,
  ...
  private class StatusIndicator(private var currentStatus: ConnectionStatus) {
    ...
    private fun showDetailsPopup() {
      var detailedErrorMessage = currentStatus.error?.toPresentableText() ?: currentStatus.additionalErrorDescription ?: ""
      ...
      val panel = JPanel(BorderLayout()).apply {
        ...
        if (throwable != null) {
          ...
          stackScrollPane.preferredSize = Dimension(stackScrollPane.preferredSize.width,
                                                    max(300, Toolkit.getDefaultToolkit().screenSize.height / 5))

          val group = HideableTitledPanel(KafkaMessagesBundle.message("testConnection.stacktrace"), true, stackScrollPane, false)
...
data class ConnectionSuccessful(override val shortDescription: String = KafkaMessagesBundle.message("connection.success")) : ConnectionStatus {
...
data class ConnectionError(override val error: Throwable?,
                           override val shortDescription: String = KafkaMessagesBundle.message("connection.error"),
                           override val additionalErrorDescription: String? = null) : ConnectionStatus {
  override val color: Color = MessageType.ERROR.titleForeground
