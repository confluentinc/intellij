...
class CredentialsHolder<D : ConnectionData> private constructor(
    private val credentialsId: CredentialId?,
    ...
    companion object {
        @RequiresEdt
        operator fun <D : ConnectionData> invoke(
            initSettings: D,
    ...
    fun wrapUsernameField(value: JBTextField): JComponent {
        ...
        if (!initializedInternal) {
            ...
            value.emptyText.appendLine(
                KafkaMessagesBundle.message("settings.credential.loading.status"),
    ...
    fun apply(conn: D) {
        ...
        val newUsername =
            usernameField?.text ?: usernameSecuredField?.password?.let(::String) ?: usernameEditorField?.text
        ...
        val modalTaskOwnerComponent =
            checkNotNull(usernameField ?: usernameSecuredField ?: usernameEditorField ?: passwordField)
        runWithModalProgressBlocking(
            ModalTaskOwner.component(modalTaskOwnerComponent),
