...
class RfsConnectionDataManager : Disposable {
    ...
    fun getConnectionsByGroupId(groupId: String, project: Project?) =
        getConnections(project).filter { it.groupId == groupId }
            ...
            val filter = this
                ...
                .filter { conn ->
                    conn.sourceConnection == null || instance?.getConnections(
                        project
                    )?.firstOrNull { conn.sourceConnection == it.innerId }?.isEnabled == true
                }
            return filter
        }
    }
