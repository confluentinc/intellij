...
class BdtKerberosManager {
    ...
    fun login(force: Boolean = false, jaasEntryName: String): LoginContext = synchronized(this) {
        ...
        val lc: LoginContext = object :
            LoginContext(jaasEntryName, Subject(), KerberosCallBackHandler(username, password), configuration) {}
        ...
        try {
            ...
            val warning =
                KafkaMessagesBundle.message("kerberos.settings.test.warning", jaasEntryName).takeIf { withRefresh }
                    ?: ""
        ...
        if (!File(cachePath).exists()) {
            throw BdtConfigurationException(
                KafkaMessagesBundle.message(
                    "kerberos.validation.cache.file.is.not.exists",
                    cachePath
                )
            )
        }
