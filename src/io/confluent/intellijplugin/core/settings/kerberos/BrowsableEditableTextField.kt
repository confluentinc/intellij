...
class BrowsableEditableTextField(project: Project) : ExtendableTextField() {
  init {
    val clearExtension = ExtendableTextComponent.Extension.create(CLOSE_ICON,
                                                                  CLOSE_HOVERED_ICON,
    ...
    addExtension(ExtendableTextComponent.Extension.create(AllIcons.General.OpenDisk,
      ...
      val file = FileChooserUtil.selectSingleFile(project, null, UIBundle.message("component.with.browse.button.accessible.name"))
                 ?: return@create
      this.text = file.path
    })

    addExtension(ExtendableTextComponent.Extension.create(AllIcons.General.Inline_edit,
                                                          AllIcons.General.Inline_edit,
                                                          UIBundle.message("button.text.edit")) {
      val vf = VirtualFileManager.getInstance().refreshAndFindFileByNioPath(Path.of(this.text)) ?: return@create
