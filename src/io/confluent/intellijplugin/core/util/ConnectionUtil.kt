...
object ConnectionUtil {
    ...
    fun enableConnectionsByData(project: Project, connectionDataList: List<ConnectionData>) {
        ...
        val res = Messages.showYesNoDialog(
            ...
            KafkaMessagesBundle.message(
                "enable.connection.confirmation.message",
    ...
    private fun showRenameDialog(connData: ConnectionData, project: Project): String? {
        return BdtMessages.showInputDialogWithDescription(
            project,
    ...
    fun removeConnectionsWithConfirmation(project: Project, selectedConnectionsIds: List<String>) {
        ...
        val messageText = KafkaMessagesBundle.message(
            if (selectedConnectionsIds.size < 2)
                "deleteConnection.single.text"
            else
                "deleteConnection.multiple.text", connectionNames
        )

        val askUser = Messages.showOkCancelDialog(
            project, messageText, "",
    ...
    fun disableConnectionsByIds(project: Project, connectionIds: List<String>) {
        val resultDataList =
            connectionIds.mapNotNull { RfsConnectionDataManager.instance?.getConnectionById(project, it) }
    ...
    private fun disableConnectionsByData(project: Project, connectionDataList: List<ConnectionData>) {
        ...
        val res = Messages.showYesNoDialog(
            project,
            KafkaMessagesBundle.message(
                "disable.connection.text", if (connectionDataList.size == 1) 0 else 1,
                connectionNames
            ), "", Messages.getQuestionIcon()
        )
        if (res == Messages.OK) {
    ...
    fun modifyConnection(project: Project?, connectionData: ConnectionData, isEnable: Boolean) {
        ...
        RfsConnectionDataManager.instance?.modifyConnection(
            project,
            connectionData,
            listOf(CommonSettingsKeys.ENABLED_KEY)
        )
    }
    ...
    fun goToConnection(project: Project, connId: String, path: RfsPath? = null) {
        invokeLater {
            ...
            val paneOwner =
                BigDataToolWindowController.getInstance(project)?.getMainPane()?.actionsOwner ?: return@invokeLater
