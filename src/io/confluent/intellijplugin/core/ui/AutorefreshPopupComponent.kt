...
class AutorefreshPopupComponent(values: List<Pair<@Nls String, Int>>? = null) : JPanel() {
    ...
    init {
        refreshAction = DumbAwareAction.create(
            KafkaMessagesBundle.message("autorefresh.refresh.action"),
    ...
    fun registerRefreshShortcut(component: JComponent) {
        refreshAction.registerCustomShortcutSet(
            KeymapUtil.getActiveKeymapShortcuts(IdeActions.ACTION_REFRESH),
            component
        )
    }
    ...
    private fun showPopupMenu() {
        val popupMenu =
            ActionManager.getInstance().createActionPopupMenu(ActionPlaces.TOOLWINDOW_POPUP, createActionGroup())
