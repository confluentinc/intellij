...
abstract class FilterPopupComponent(@Nls label: String) : JPanel() {
    ...
    protected open val defaultSelectorForeground: Color
        get() = if (UIUtil.isUnderDarcula()) UIUtil.getLabelForeground() else NamedColorUtil.getInactiveTextColor()
            .darker().darker()
    ...
    private fun setDefaultForeground() {
        if (nameLabel != null) {
            nameLabel!!.foreground =
                if (UIUtil.isUnderDarcula()) UIUtil.getLabelForeground() else NamedColorUtil.getInactiveTextColor()
        }
    ...
    private fun setOnHoverForeground() {
        nameLabel?.foreground =
            if (UIUtil.isUnderDarcula()) UIUtil.getLabelForeground() else UIUtil.getTextAreaForeground()
    ...
    private fun createPopupMenu(): ListPopup {
        return JBPopupFactory.getInstance().createActionGroupPopup(
            null, createActionGroup(), DataManager.getInstance().getDataContext(this),
        ...
        override fun paintBorder(c: Component?, g: Graphics?, x: Int, y: Int, width: Int, height: Int) {
            ...
            val area = Area(
                RoundRectangle2D.Double(
                    x.toDouble(), y.toDouble(), width.toDouble(), height.toDouble(), myArcSize.toDouble(),
                    myArcSize.toDouble()
                )
            )
            ...
            area.subtract(
                Area(
                    RoundRectangle2D.Double(
                        (x + thickness).toDouble(), (y + thickness).toDouble(),
