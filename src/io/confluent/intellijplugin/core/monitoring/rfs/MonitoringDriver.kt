package io.confluent.intellijplugin.core.monitoring.rfs

import com.intellij.openapi.Disposable
import com.intellij.openapi.project.Project
import io.confluent.intellijplugin.core.connection.tunnel.model.RestClientData
import io.confluent.intellijplugin.core.monitoring.connection.MonitoringFileInfoManager
...
import io.confluent.intellijplugin.core.rfs.tree.DriverWithCompoundsTreeModel
...
abstract class MonitoringDriver(override val project: Project?, val testConnection: Boolean) : AbstractDriver(),
    Disposable {
    ...
    override val treeNodeBuilder: RfsDriverTreeNodeBuilder = object : RfsDriverTreeNodeBuilder() {
        override fun createNode(project: Project, path: RfsPath, driver: Driver) =
            MonitoringRfsTreeNode(project, root, this@MonitoringDriver)
    }
    ...
    open fun doLoadFileInfo(rfsPath: RfsPath): FileInfo? = DummyFileInfo(
        "", root, root.stringRepresentation(),
    ...
    override fun createRfsPath(path: String): RfsPath =
        RfsPath(path.split("/").filter { it.isNotBlank() }, path.endsWith("/"))
    ...
    open fun doCreateWriteStream(rfsPath: RfsPath, overwrite: Boolean, create: Boolean): OutputStream =
