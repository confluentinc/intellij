...
class BdtMonitoringUpdater(val dataManager: MonitoringDataManager) : Disposable {
  ...
  fun rescheduleAll() = invokeLater {
    ...
    if (dataManager.settings.dataUpdateIntervalMillis > 0)
      alarm.addRequest({ runBlockingMaybeCancellable { reloadAll() } },
                       dataManager.settings.dataUpdateIntervalMillis,
                       true)
  }


  private fun innerRefresh(models: List<DataModel<*>>, isCurrentCanceled: AtomicBooleanProperty, withConnectionCheck: Boolean = false) {
    ...
    try {
      ...
      toUpdate.withIndex().forEach { indexedValue ->
        ...
        notify {
          it.setText(id, KafkaMessagesBundle.message("monitoring.progress.update.data", indexedValue.index + 1))
        }
  ...
  fun checkConnectionOrRefresh(calledByUser: Boolean,
  ...
  class UpdateTask(val id: Int = Random.nextInt(), val update: (AtomicBooleanProperty) -> Unit) : Comparable<UpdateTask> {
