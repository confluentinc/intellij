...
@Service
@State(
    name = "ConfluentIntellijKafkaRfsFileViewerSettings",
    storages = [Storage("confluent-kafka-rfs-file-viewer-settings.xml")]
)
class RfsFileViewerSettings : PersistentStateComponent<RfsFileViewerSettings> {
    ...
    data class RfsFileViewerInfo(
        var driverId: String = "", // Actually - connection ID
    ...
    private fun Driver.connectionDataClassName(): String =
        RfsConnectionDataManager.instance?.findConnectionInAllProjects(getExternalId())?.javaClass?.name
            ?: javaClass.name
}