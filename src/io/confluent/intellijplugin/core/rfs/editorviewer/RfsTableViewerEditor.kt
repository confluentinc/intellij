...
import io.confluent.intellijplugin.core.rfs.driver.metainfo.details.getComponent
...
import io.confluent.intellijplugin.core.ui.setCenterComponent
...
class RfsTableViewerEditor(val project: Project, val virtualFile: VirtualFile) : FileEditor, UserDataHolderBase() {
    ...
    private fun createToolbar(targetComponent: JComponent): JPanel {
        ...
        val detailsToggle = object : DumbAwareToggleAction(
            KafkaMessagesBundle.message("file.viewer.toggle.details"), null,
            AllIcons.Actions.PreviewDetails
        ) {
            override fun isSelected(e: AnActionEvent) = RfsFileViewerSettings.getInstance().showDetailsPanel
            override fun getActionUpdateThread() = ActionUpdateThread.BGT
            override fun setSelected(e: AnActionEvent, state: Boolean) {
                RfsFileViewerSettings.getInstance().showDetailsPanel =
                    !RfsFileViewerSettings.getInstance().showDetailsPanel
                updateDetails()
            }
        }

        val rightToolbar = ToolbarUtils.createActionToolbar(
            targetComponent = targetComponent, RfsActionPlaces.RFS_VIEWER_EDITOR,
            DefaultActionGroup(detailsToggle), true
        ).apply {
