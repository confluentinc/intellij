...
open class RfsDriverTreeNodeBuilder {
    fun getNode(
        ...
        parent: DriverFileRfsTreeNode? = null
    ): DriverFileRfsTreeNode {
    ...
    fun getLoadMoreNode(
        ...
        nextMarker: RfsListMarker,
        loadMoreAction: DriverRfsLoadMoreNode.() -> Unit
    ): DriverRfsLoadMoreNode {
        val node = DriverRfsLoadMoreNode(
            treeNode.driver,
            treeNode.project,
            RfsChildrenPartId(treeNode.rfsPath, nextMarker),
            loadMoreAction
        )
        node.update()
        return node
    ...
    protected open fun createNode(
        ...
        path: RfsPath,
        driver: Driver
    ) = DriverFileRfsTreeNode(project, path, driver)
}