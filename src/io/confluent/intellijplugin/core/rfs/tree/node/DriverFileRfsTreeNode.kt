...
open class DriverFileRfsTreeNode(
    project: Project,
    val rfsPath: RfsPath,
    driver: Driver
    ...
    var loadingNode: DriverRfsLoadingNode?
        get() = (serviceNode as? DriverRfsLoadingNode).takeIf {
            this.updatingChildrenTask.get().let { it != null && !it.refresh && !it.isExpired() }
        }
        private set(value) {
            serviceNode = value
        }
    ...
    fun loadChildrenIfNotRunning(
        loadingNode: DriverRfsLoadingNode? = null,
    ...
    fun addLoadedChildren(
        newChildren: List<DriverFileRfsTreeNode>,
    ...
    override fun update(presentation: PresentationData) {
        ...
        fileInfo?.let {
            ...
            if (it.isFile && size > 1 && modified > 1)
                presentation.tooltip = KafkaMessagesBundle.message(
                    "rfs.node.file.tooltip", SizeUtils.toString(size),
