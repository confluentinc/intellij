...
import io.confluent.intellijplugin.core.rfs.tree.node.DriverRfsTreeNode
import io.confluent.intellijplugin.core.rfs.tree.node.RfsMetaInfoNode
...
open class DriverRfsTreeModel(
    val project: Project,
    val rootPath: RfsPath,
    val driver: Driver,
    private val enableLoadMore: Boolean = true
) : BaseTreeModel<RfsTreeNode>(), InvokerSupplier {
    ...
    private fun getFileInfoChildren(treeNode: DriverFileRfsTreeNode): List<DriverRfsTreeNode> {
        ...
        if (treePath == null) {
            ...
            treeNode.loadChildrenIfNotRunning(
                refresh = treeNode.cachedChildren != null,
    ...
    private fun addLoadedChildren(
        treePath: TreePath,
        treeNode: DriverFileRfsTreeNode,
        calculatedChildren: SafeResult<RfsFileInfoChildren>,
        append: Boolean = false
    ) {
        val newChildrenFileInfos =
            preprocessLoadedChildren(calculatedChildren.result?.fileInfos?.toList() ?: emptyList())
        ...
        val loadMoreNode = if (enableLoadMore && nextMarker?.marker != null)
            oldLoadMoreNode ?: this.nodeBuilder.getLoadMoreNode(treeNode, nextMarker) {
                loadMore(
                    treePath,
                    treeNode,
                    nextMarker
        ...
        if (append && calculatedChildren.result?.fromMarker != null && oldChildrenFileInfos != null) {
            ...
            treeNodesInserted(
                treePath,
                newChildrenNodes.indices.map { oldChildrenNodes.size + it }.toIntArray(),
                newChildrenNodes.toTypedArray()
            )
        } else if (calculatedChildren.result?.fromMarker == null) {
            if (!treeNode.isInvalidated.get()) {
                val oldChildrenNodesByFileInfo =
                    oldChildrenNodes?.mapNotNull { it.fileInfo?.to(it) }?.toMap() ?: emptyMap()
                val updatedChildrenNodes =
                    newChildrenNodes.mapIndexed { i, it -> oldChildrenNodesByFileInfo[it.fileInfo] ?: it }
    ...
    private fun updateNode(fileInfo: FileInfo?, error: Throwable?, treePath: TreePath) {
        ...
        if (node.fileInfo == fileInfo && error?.toPresentableText() == node.error?.toPresentableText()) {
            log(
                node.rfsPath,
                "Ignore node update ${System.identityHashCode(node)}. The same. Loading: ${node.isLoading}"
            )
            return
        }
