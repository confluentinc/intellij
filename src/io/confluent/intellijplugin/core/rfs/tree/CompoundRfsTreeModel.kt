...
class CompoundRfsTreeModel(
    val project: Project,
    val getRootSources: () -> List<Pair<String, RfsPath?>>,
    private val additionalDrivers: List<Driver> = emptyList(),
    val preprocessChildren: (List<RfsTreeNode>) -> List<RfsTreeNode> = { it }
) : BaseTreeModel<RfsTreeNode>(),
    InvokerSupplier {
    ...
    fun getCachedDriverTreePath(driver: Driver, rfsPath: RfsPath): TreePath? {
        ...
        val masterModel =
            driverTreeModels.firstOrNull { it.driver.getExternalId() == masterConnectionId } ?: return null
        val masterDriver = masterModel.driver
        val masterPath =
            (masterDriver as MasterDriver).getDependConnectionRfsPath(dependConnectionData.innerId) ?: return null
        val masterTreePath = getCachedDriverTreePath(masterDriver, masterPath) ?: return null

        val compoundTreeSource =
            (masterModel as DriverWithCompoundsTreeModel).compoundsModels[masterPath] ?: return null
        val slavePath = compoundTreeSource.treeModel.getCachedDriverTreePath(driver, rfsPath) ?: return null

        val nodes = TreePathUtil.convertTreePathToArray(masterTreePath).toList() + TreePathUtil.convertTreePathToArray(
            slavePath
        ).toList().drop(
            1
        )
    ...
    private fun addNewDisabledNodes(newRoots: List<Pair<String, RfsPath?>>) {
        val newDisabledConn = newRoots.mapNotNull {
            val connectionData =
                RfsConnectionDataManager.instance?.getConnectionById(project, it.first) ?: return@mapNotNull null
    ...
    private fun initDriverTreeModel(
        driver: Driver,
        rfsPath: RfsPath
    ): DriverRfsTreeModel {
        ...
        val treeListener = object : TreeModelListener {
            override fun treeNodesChanged(e: TreeModelEvent) =
                this@CompoundRfsTreeModel.treeNodesChanged(
                    convertDriverTreePath(e.treePath),
                    e.childIndices,
                    e.children
                )

            override fun treeNodesInserted(e: TreeModelEvent) =
                this@CompoundRfsTreeModel.treeNodesInserted(
                    convertDriverTreePath(e.treePath),
                    e.childIndices,
                    e.children
                )
            ...
            override fun treeNodesRemoved(e: TreeModelEvent) =
                this@CompoundRfsTreeModel.treeNodesRemoved(
                    convertDriverTreePath(e.treePath),
                    e.childIndices,
                    e.children
                )
            ...
            override fun treeStructureChanged(e: TreeModelEvent) =
                this@CompoundRfsTreeModel.treeStructureChanged(
                    convertDriverTreePath(e.treePath),
                    e.childIndices,
                    e.children
                )
    ...
    private fun createDisabledNode(connectionData: RemoteFsDriverProvider) =
        DisabledRfsTreeNode(project, connectionData)
