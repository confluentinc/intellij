...
open class DriverWithCompoundsTreeModel(
    project: Project,
    rootPath: RfsPath,
    driver: Driver
) : DriverRfsTreeModel(project, rootPath, driver, enableLoadMore = false) {
    ...
    override fun getChildren(parent: Any?): List<RfsTreeNode>? =
        if (parent is DriverCompoundTreeNode && parent.isCompound)
    ...
    private fun createChildListener(parent: DriverFileRfsTreeNode) = object : TreeModelListener {
        override fun treeNodesChanged(e: TreeModelEvent) =
            this@DriverWithCompoundsTreeModel.treeNodesChanged(
                convertDriverTreePath(parent, e.treePath),
                e.childIndices,
                e.children
            )

        override fun treeNodesInserted(e: TreeModelEvent) =
            this@DriverWithCompoundsTreeModel.treeNodesInserted(
                convertDriverTreePath(parent, e.treePath),
                e.childIndices,
                e.children
            )
        ...
        override fun treeNodesRemoved(e: TreeModelEvent) =
            this@DriverWithCompoundsTreeModel.treeNodesRemoved(
                convertDriverTreePath(parent, e.treePath),
                e.childIndices,
                e.children
            )
        ...
        override fun treeStructureChanged(e: TreeModelEvent) =
            this@DriverWithCompoundsTreeModel.treeStructureChanged(
                convertDriverTreePath(parent, e.treePath),
                e.childIndices,
                e.children
            )
    ...
    private fun convertDriverTreePath(parent: DriverFileRfsTreeNode, driverPath: TreePath): TreePath? {
        ...
        @Suppress("UNCHECKED_CAST") val newPath =
            TreePathUtil.convertTreePathToArray(driverPath).toList() as List<AbstractTreeNode<*>>
        prevPath.reverse()
