...
class SafeExecutor(val coroutineScope: CoroutineScope, private val defaultTimeout: Duration) : Disposable {
    ...
    fun <T> asyncInterruptible(
        @NlsSafe taskName: String?,
    ...
    private suspend fun <T> computeDetachedWithBackgroundProgress(
        showProgress: ProgressOptions,
        body: suspend CoroutineScope.() -> T
    ): T {
        ...
        try {
            ...
            coroutineScope.launch(Dispatchers.IO) {
                withBackgroundProgress(
                    project,
                    KafkaMessagesBundle.message("driver.task.canceling.progress.title", title),
    ...
    private suspend fun <T> withBackgroundProgress(
        ...
        when {
            projects.size == 1 -> return com.intellij.platform.ide.progress.withBackgroundProgress(
                projects.first(),
                title,
                cancellation,
                action
    ...
    companion object {
        ...
        private suspend fun logDetached(
            hangingScope: CoroutineScope,
            title: String,
            ...
            select {
                ...
                onTimeout(giveUpTimeout) {
                    ...
                    thisLogger().error(
                        "SafeExecutor had to detach cancelling job $title. Not finished after $giveUpTimeout. Coroutine dump:\n$newDump",
                        e
                    )
                }
            }
        ...
        private fun createInstance(
            ...
            val childScope = service<SafeExecutorService>().coroutineScope.childScope(
                name,
