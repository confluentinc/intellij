...
import io.confluent.intellijplugin.core.rfs.driver.fileinfo.toKotlinResult
...
import io.confluent.intellijplugin.core.rfs.driver.metainfo.DriverFileMetaInfoProviderBase
import io.confluent.intellijplugin.core.rfs.fileInfo.*
import io.confluent.intellijplugin.core.rfs.fileInfo.impl.DriverCacheFileInfoManager
...
import io.confluent.intellijplugin.core.rfs.tree.SmartDriverRfsTreeModel
import io.confluent.intellijplugin.core.rfs.tree.node.RfsDriverTreeNodeBuilder
import io.confluent.intellijplugin.core.rfs.url.UrlFileOpener
import io.confluent.intellijplugin.core.util.*
import io.confluent.intellijplugin.core.util.async.runOrAwait
...
abstract class DriverBase : AbstractDriver() {
    ...
    private val fileInfoLoader: DriverFileInfoLoader by lazy {
        object : DriverFileInfoLoader(project, this@DriverBase) {
            ...
            override fun doLoadChildrenFileInfos(id: RfsChildrenPartId): Flow<RfsFileInfoChildren> =
                this@DriverBase.doListStatus(id)
    ...
    override fun createTreeModel(rootPath: RfsPath, project: Project): DriverRfsTreeModel =
        SmartDriverRfsTreeModel(project, rootPath, this)

    private val innerConnectionStatus =
        MutableStateFlow<DriverConnectionStatus>(FailedConnectionStatus(BdtDriverNotInitializedException()))
    ...
    override fun getWriteStream(
        rfsPath: RfsPath,
        overwrite: Boolean,
        canCreateNewFile: Boolean
    ): SafeResult<OutputStream> =
        safeExecutor.asyncInterruptible<OutputStream>(
            taskName = "Opening write stream for '$rfsPath'",
            timeout = timeout * 2
        ) {
            if (rfsPath.isDirectory) {
                throw IllegalArgumentException(
                    KafkaMessagesBundle.message(
                        "rfs.error.no.read.stream.for.directory",
                        rfsPath
                    )
                )
            }
