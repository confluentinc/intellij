...
class DummyFileInfo(
    ...
    private fun noDelegateResult() =
        IllegalStateException(KafkaMessagesBundle.message("rfs.dummy.info.no.delegate", externalPath))

    //TODO do we want to successfully construct tasks when there is no delegate status?
    override fun deleteAsync(): SafeResult<RemoteFsTask> =
        delegateStatus()?.deleteAsync() ?: ErrorResult(noDelegateResult())

    override fun renameAsync(newPath: RfsPath, overwrite: Boolean): SafeResult<RemoteFsMoveTask> =
        delegateStatus()?.renameAsync(
            newPath,
            overwrite
        )
