...
class DirectorySizeAndCountComponentController(
    val project: Project,
    ...
    private fun start() {
        ...
        context.progressNotifier.plusAssign { totalCount, totalSize ->
            val message = KafkaMessagesBundle.message(
                "rfs.directory.task.estimate.contents.info",
                totalCount,
    ...
    private fun calculateDirectoryInfo(): RfsEstimateSizeCountContext {
        val task = object :
            Task.Backgroundable(project, KafkaMessagesBundle.message("rfs.directory.task.estimate.contents.title")) {
            override fun run(indicator: ProgressIndicator) {
                val curContext =
                    (indicator as BackgroundableProcessIndicator).getUserData(RfsEstimateSizeCountContext.KEY)!!
                try {
