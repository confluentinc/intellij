...
open class FileInfoDetailsBase(val rfsTreeNode: DriverFileRfsTreeNode, parentDisposable: Disposable) : FileInfoDetails {
    ...
    override fun getBlocks(): List<FileInfoBlock> {
        ...
        val panel = MigPanel(UiUtil.insets10FillXHidemode3).apply {
            ...
            row(
                KafkaMessagesBundle.message("file.info.label.type"),
                SelectableLabel(FileMetaInfoUtils.getPathType(path))
            )
            ...
            rowIfNotBlank(
                KafkaMessagesBundle.message("file.info.label.modification.time"),
                FileMetaInfoUtils.getFileModificationTime(fileInfo)
            )
            rowIfNotBlank(
                KafkaMessagesBundle.message("file.info.label.permissions"),
                fileInfo.permission?.printString()
            )
