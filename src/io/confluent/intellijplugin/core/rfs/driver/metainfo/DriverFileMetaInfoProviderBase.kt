...
open class DriverFileMetaInfoProviderBase(open val driver: Driver) : DriverFileMetaInfoProvider {
    ...
    override fun getAllTableColumns(): List<RfsTableColumn<*>> = listOf(
        ...
        object : RfsTableColumn<String>(
            KafkaMessagesBundle.message("column.name.modification.time"),
            MODIFIED_COLUMN,
            columns = 19
        ) {
            override fun valueForFileInfo(fileInfo: FileInfo): String =
    ...
    override fun getFileDetails(
        rfsTreeNode: DriverFileRfsTreeNode,
        curWindowDisposable: Disposable
    ) = FileInfoDetailsBase(rfsTreeNode, curWindowDisposable)
