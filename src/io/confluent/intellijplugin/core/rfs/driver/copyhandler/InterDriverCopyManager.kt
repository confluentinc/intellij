...
object InterDriverCopyManager {
    ...
    fun canCopyToFolder(
        sourceFileInfos: Iterable<FileInfo>,
        targetPath: RfsPath,
        targetDriver: Driver,
        overwrite: Boolean
    ): Boolean {
        if (!targetPath.isDirectory)
            return false
        val isAnyContainsYet =
            sourceFileInfos.all { it.driver == targetDriver } && sourceFileInfos.any { targetPath.startsWith(it.path) }
        if (isAnyContainsYet)
            return false
    ...
    private fun getStreamHandler(
        fromInfo: FileInfo,
        toDriver: Driver
    ): HostPassThroughCopyHandler? {
