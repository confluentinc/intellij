...
import io.confluent.intellijplugin.core.rfs.copypaste.dialog.RfsSkipOverwriteChoice
...
data class RfsCopyMoveContext(val project: Project, val isCopy: Boolean) {
    ...
    fun notifyIndicator() {
        ...
        progressIndicator?.text2 = when {
            ...
            isCopy && fileSize == -1L -> KafkaMessagesBundle.message(
                "copy.undefined.target",
                SizeUtils.toString(proceededFileBytes)
            )

            isCopy -> KafkaMessagesBundle.message(
                "copy.progress",
                SizeUtils.toString(proceededFileBytes),
                SizeUtils.toString(fileSize)
            )

            fileSize == -1L && proceededFileBytes == 0L -> KafkaMessagesBundle.message("copy.undefined")
            fileSize == -1L -> KafkaMessagesBundle.message(
                "copy.undefined.target",
                SizeUtils.toString(proceededFileBytes)
            )

            else -> KafkaMessagesBundle.message(
                "copy.progress",
                SizeUtils.toString(proceededFileBytes),
                SizeUtils.toString(fileSize)
            )
        ...
        when {
            ...
            fileSize == -1L -> progressIndicator?.fraction = processedFiles.toDouble() / totalCount
                ...
                progressIndicator?.fraction =
                    (processedFiles - 1).toDouble() / totalCount + (proceededFileBytes).toDouble() / fileSize / totalCount
            }
        }
    }
    ...
    private fun notifyFastIndicator() {
        ...
        progressIndicator?.text =
            targetFileName?.let { KafkaMessagesBundle.message("rfs.copy.process.text.fast", sourceFileName, it) }
                ?: sourceFileName
    ...
    companion object {
        fun createSingle(
            ...
            val rfsCopyMoveContext = RfsCopyMoveContext(
                project = project,
                isCopy = isCopy
            )
