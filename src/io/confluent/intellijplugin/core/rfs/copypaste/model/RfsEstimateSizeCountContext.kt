...
data class RfsEstimateSizeCountContext(val progressIndicator: ProgressIndicator) {
  ...
  private fun updateIndicator() {
    ...
    progressIndicator.text = KafkaMessagesBundle.message("rfs.directory.task.estimate.contents.progress.text",
                                                    curFilePath)
    if (totalSize != -1L) {
      progressIndicator.text2 = KafkaMessagesBundle.message("rfs.directory.task.estimate.contents.info", totalCount,
                                                            SizeUtils.toString(totalSize))

    }
    else {
      progressIndicator.text2 = KafkaMessagesBundle.message("rfs.directory.task.estimate.contents.info.without.size", totalCount)
    }
