...
object RfsCopyPasteHelpers {
    ...
    fun resolveOverwriteFileInfo(
        toDriver: Driver,
        toPath: RfsPath,
        context: RfsCopyMoveContext
    ) = resolveOverwrite(toPath.stringRepresentation(), context) {
        toDriver.getFileStatus(toPath, force = true).resultOrThrow() != null
    }


    fun resolveOverwriteBucketable(
        toBucket: String,
        toKey: String,
        context: RfsCopyMoveContext,
        checkIsExists: () -> Boolean
    ) = resolveOverwrite("$toBucket/$toKey", context) {
        checkIsExists()
    }
    ...
    fun resolveOverwrite(
        ...
        val choice = RfsSkipOverwriteChoice.askUser(
            context.project, toPath, toPath.split("/").last(),
    ...
    fun copyStreams(
        ...
        val toStream =
            toDriver.getWriteStream(toPath, overwrite = true, canCreateNewFile = true).resultOrThrowWithWrapper(
                KafkaMessagesBundle.message(
                    "copy.through.local.file.failed.create.write.stream",
                    toPath,
