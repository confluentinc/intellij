  ...
  fun createDialogLabel(sourceFileInfos: List<FileInfo>, pasteType: CopyOrMove): @Nls String {
    ...
    return if (isSingleFileInfo) {
      when (Pair(firstFileInfo.isFile, pasteType)) {
        ...
        Pair(true, CopyOrMove.COPY) -> RefactoringBundle.message("copy.files.copy.file.0", firstFileInfo.externalPath)
        Pair(false, CopyOrMove.MOVE) -> RefactoringBundle.message("move.directory.0", firstFileInfo.externalPath)
