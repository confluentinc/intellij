...
import io.confluent.intellijplugin.core.rfs.copypaste.utils.RfsUserConfirmatory
...
class CopyMoveTask(
    ...
    private val onFileLoad: suspend (List<RfsPath>) -> Unit
) : Task.Backgroundable(project, "CalculateLater") {
    init {
    ...
    private fun move(indicator: ProgressIndicator) {
        ...
        val canContinue =
            RfsUserConfirmatory.askCanContinue(project, userInfoMessage, sourceFileInfos.first().driver.getExternalId())
        if (!canContinue)
            return
    ...
    private fun copy(indicator: ProgressIndicator) {
        ...
        sourceFileInfos.forEach { sourceFileInfo ->
            if (indicator.isCanceled)
                return
            val copyTask = RfsCopyPasteService.createCopyTaskToFolder(
                sourceFileInfo,
                targetFolderPath,
                targetDriver,
                exportFormat,
