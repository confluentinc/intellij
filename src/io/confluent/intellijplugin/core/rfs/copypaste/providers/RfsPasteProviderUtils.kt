...
import io.confluent.intellijplugin.core.rfs.copypaste.RfsCopyPasteManager
import io.confluent.intellijplugin.core.rfs.copypaste.model.TransferableDescriptor
import io.confluent.intellijplugin.core.rfs.copypaste.model.rfsDataFlavor
...
object RfsPasteProviderUtils {
    fun processRfsPaste(
        ...
        RfsCopyPasteManager.copyMoveWithDialog(
            project = project,
    ...
    internal fun getTransferableDescriptor() =
        CopyPasteManagerEx.getInstanceEx().getContents<TransferableDescriptor>(rfsDataFlavor)
    ...
    private fun getFileInfoFromString(): List<FileInfo>? {
        val files =
            CopyPasteManagerEx.getInstanceEx().getContents<List<File>>(DataFlavor.javaFileListFlavor) ?: return null
        return files.filter { it.parentFile != null }.map { LocalDriverManager.instance.createFileInfo(it) }
            .ifEmpty { null }
    }
}