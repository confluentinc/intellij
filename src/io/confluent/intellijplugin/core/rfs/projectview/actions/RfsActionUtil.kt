...
object RfsActionUtil {
    ...
    fun excludeNestedPaths(paths: List<TreePath>): Set<TreePath> =
        paths.sortedBy { it.pathCount }.fold(emptySet()) { mySet, newPath ->
    ...
    fun filesAndDirectoriesString(files: List<FileInfo>) =
        if (files.size == 1)
            ...
            if (simpleFiles > 0) {
                result.append(simpleFiles).append(" ")
                    .append(IdeBundle.message("prompt.delete.file", plural(simpleFiles)))
            }
            if (symlinks > 0) {
