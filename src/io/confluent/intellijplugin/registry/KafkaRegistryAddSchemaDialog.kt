...
class KafkaRegistryAddSchemaDialog(project: Project, val dataManager: BaseClusterDataManager) :
    ...
    private val panel = panel {
        ...
        if (isConfluentSchema) {
            row(KafkaMessagesBundle.message("schema.registry.add.schema.dialog.field.key.value")) {
                cell(
                    keyValueCombobox
                )
            }.visibleIf(
            ...
            row(KafkaMessagesBundle.message("schema.registry.add.schema.dialog.field.topic")) { cell(topicField) }.visibleIf(
                topicFieldVisible
            )
    ...
    override fun getDimensionServiceKey(): String =
        "io.confluent.intellijplugin.core.ui.add.kafka.registry.dialog.bounds"
        ...
        when (getStrategy()) {
            ConfluentRegistryStrategy.CUSTOM -> {
                ...
                subjectNameLabel.text =
                    KafkaMessagesBundle.message("schema.registry.add.schema.dialog.field.custom.name")
            ...
            ConfluentRegistryStrategy.TOPIC_NAME -> {
                ...
                subjectNameLabel.text =
                    KafkaMessagesBundle.message("schema.registry.add.schema.dialog.field.computed.name")
                keyValueVisible.set(true)
                topicFieldVisible.set(true)
                recordFieldVisible.set(false)

                subjectNameField.text =
                    topicField.item?.name?.ifBlank { "<topic>" } + "-" + keyValueCombobox.item?.name?.lowercase()
            }
            ...
            ConfluentRegistryStrategy.TOPIC_RECORD_NAME -> {
                ...
                updateRecordFieldText()
                subjectNameField.text =
                    topicField.item?.name?.ifBlank { "<topic>" } + "-" + recordField.text?.ifBlank { "<record>" }
            }
        }
    }
    ...
    private fun updateRecordFieldText() {
        ...
        if (strategy == ConfluentRegistryStrategy.TOPIC_RECORD_NAME) {
            val newRecordName = KafkaRegistryUtil.parseRecordName(cachedParsedSchema) ?: ""
            subjectNameField.text =
                topicField.item?.name?.ifBlank { "<topic>" } + "-" + newRecordName.ifBlank { "<record>" }
        }
    }
