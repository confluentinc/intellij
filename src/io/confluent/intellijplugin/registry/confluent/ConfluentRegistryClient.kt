...
class ConfluentRegistryClient(restService: RestService, props: Map<String, String>) : Disposable {
    val internalClient = CachedSchemaRegistryClient(
        restService,
        100,
    ...
    fun listSchemas(
        ...
        val names =
            internalClient.getAllSubjects(registryShowDeletedSubjects)?.sortedBy { it.lowercase() } ?: emptyList()
        ...
        return filteredNames.map { KafkaSchemaInfo(it) }.sortedSchemas(connectionId).take(
            limit ?: Int.MAX_VALUE
        ) to (limit != null && names.size > limit)
    }
    ...
    fun updateSchema(registryInfo: SchemaVersionInfo, newText: @NlsSafe String) {
        val parsedSchema =
            KafkaRegistryUtil.parseSchema(registryInfo.type, newText, this, registryInfo.references).getOrThrow()
        try {
    ...
    companion object {
        ...
        private fun createConfluentClient(
            ...
            val brokerSsl = if (connectionData.registryUseBrokerSsl) {
                ...
                mapOf()
            val registryProps =
                BdtPropertyComponent.parseProperties(connectionData.secretRegistryProperties).associate {
                    (it.name ?: "") to (it.value ?: "")
                }
            ...
            val tunnel = BdtSshTunnelService.createIfRequired(
                project, connectionData.getTunnelData().copy(localPort = null),
