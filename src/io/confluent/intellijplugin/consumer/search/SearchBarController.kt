 ...
 * case-insensitive [String.contains] on the cell's string value.
 *
...
class SearchBarController(
    ...
    private fun columnContainsFilter(needle: String, modelIndex: Int): RowFilter<TableModel, Int> =
        object : RowFilter<TableModel, Int>() {
            override fun include(entry: Entry<out TableModel, out Int>): Boolean =
                entry.getStringValue(modelIndex).contains(needle, ignoreCase = true)
        }
        ...
        private fun defaultRowSnapshot(table: JTable): List<SearchableSlot> {
            ...
            for (row in 0 until rowCount) {
                ...
                for (col in 0 until model.columnCount) {
                    builder.append(model.getValueAt(row, col)?.toString() ?: "")
                    builder.append(' ')
                }
