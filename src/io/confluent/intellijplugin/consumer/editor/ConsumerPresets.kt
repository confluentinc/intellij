...
class RunConsumerConfigCellRenderer : ListCellRenderer<StorageConsumerConfig> {
    ...
    private val labels = listOf(
        topicLabel, keyLabel, valueLabel, startFromLabel, limitLabel, filterLabel, filterKeyLabel, filterValueLabel,
    ...
    override fun getListCellRendererComponent(
        list: JList<out StorageConsumerConfig>,
        ...
        topicLabel.text = KafkaMessagesBundle.message(
            "consumer.preset.topic", if (value.topic.isNullOrEmpty()) KafkaMessagesBundle.message(
        ...
        if (limit.type == ConsumerLimitType.NONE) {
            ...
            limitLabel.text = KafkaMessagesBundle.message(
                "consumer.preset.limit",
                limit.type.title,
        ...
        if (filterHeaderKeyLabel.isVisible)
            filterHeaderKeyLabel.text =
                KafkaMessagesBundle.message("consumer.preset.header.key", filter.filterHeadKey ?: "")
        if (filterHeaderValueLabel.isVisible)
            filterHeaderValueLabel.text =
                KafkaMessagesBundle.message("consumer.preset.header.value", filter.filterHeadValue ?: "")
