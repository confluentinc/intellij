...
object PrimitivesGenerator {
    ...
    fun generateFloat(from: Float = Float.MIN_VALUE, until: Float = Float.MAX_VALUE): Float =
    ...
    fun generateBytes(minSize: Int = 10, maxSize: Int = 100): ByteArray =
        Random.nextBytes(Random.nextInt(minSize, maxSize))

    fun generateInt(from: Int? = Int.MIN_VALUE, until: Int? = Int.MAX_VALUE): Int =
        Random.nextInt(from ?: Int.MIN_VALUE, until ?: Int.MAX_VALUE)

    fun generateUlong(from: ULong = ULong.MIN_VALUE, until: ULong = ULong.MAX_VALUE): ULong =
