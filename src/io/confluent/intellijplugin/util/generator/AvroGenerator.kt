...
class AvroGenerator private constructor(private val topLevelSchema: Schema) {
    ...
    private fun <L : LogicalType?> getLogicalType(
        schema: Schema,
        logicalTypeName: String,
        logicalTypeClass: Class<L>
    ): L? {
    ...
    private fun getDecimalLogicalType(schema: Schema) =
        getLogicalType(schema, DECIMAL_LOGICAL_TYPE_NAME, LogicalTypes.Decimal::class.java)
    ...
    private fun parseIterations(schema: Schema, propertiesProp: Map<*, *>): Iterator<Any> {
        ...
        when (schema.type) {
            ...
            Schema.Type.STRING -> return createStringIterator(getIntegerIterator(iterationProp), propertiesProp)
                ...
                String.format(
                    "%s property can only be specified on numeric, boolean or string schemas, not %s schema",
                    ITERATION_PROP,
    ...
    private class IntegralIterator(
        start: Long, restart: Long,
        step: Long,
        initial: Long,
        count: Long,
