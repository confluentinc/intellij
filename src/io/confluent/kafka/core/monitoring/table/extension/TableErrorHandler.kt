 ...
 */
class TableErrorHandler<T : RemoteInfo> private constructor(private val table: DataTable<T>,
                                                            private val customTableEventListener: TableEventListener? = null) : TableEventListener, Disposable {
  companion object {
    ...
    table.emptyText.apply {
      ...
      if (!message.isNullOrBlank()) {
        ...
        appendLine(linkText, SimpleTextAttributes.LINK_ATTRIBUTES) {
          ...
          val pane = IdeTooltipManager.initPane(message,
                                                HintHint()
        ...
        appendLine(KafkaMessagesBundle.message("table.loading.error.secondary"), SimpleTextAttributes.GRAYED_ATTRIBUTES, null)
      }
    }
    ...
    if (provider != null) {
      ...
      if (disableFilteredMessage) {
        ...
        if (table.tableModel.rowCount == 0) {
          ...
          table.emptyText.appendText(KafkaMessagesBundle.message("table.loaded.data", table.tableModel.rowCount))
          table.emptyText.appendSecondaryText(KafkaMessagesBundle.message("table.loaded.data.secondary"), SimpleTextAttributes.GRAYED_ATTRIBUTES,
                                              null)
        }
      }
    }
