...
class TableResizeController private constructor(private val table: JTable) : TableModelListener {
  ...
  companion object {
    fun getFor(table: JTable): TableResizeController? = (table.model as? AbstractTableModel)?.getListeners(
      TableResizeController::class.java)?.firstOrNull()
  ...
  private val columnListener = object : TableColumnModelListener {
    ...
    override fun columnMoved(e: TableColumnModelEvent) {
      ...
      (table.columnModel.getColumn(e.fromIndex).cellRenderer as? MaterialTableCellRenderer)?.lastColumn = e.fromIndex == lastColumn
      (table.columnModel.getColumn(e.toIndex).cellRenderer as? MaterialTableCellRenderer)?.lastColumn = e.toIndex == lastColumn
    }
  ...
  fun componentResized() {
    ...
    if (mode == Mode.STRETCH_LAST_COLUMN) {
      ...
      val nonPriorPreferredWidth = tableColumnsList.filter { !priorColumns.contains(it.headerValue) }.sumOf { it.preferredWidth }
