...
class TableLoadingDecorator private constructor(private val table: JBTable, @Nls text: String) : TableModelListener, Disposable {

  companion object {
    fun installOn(table: JBTable, parentDisposable: Disposable,
                  @Nls text: String = KafkaMessagesBundle.message("table.loading")): TableLoadingDecorator? {

      return if (table.model.rowCount != 0 ||
                 (table.model as? AbstractTableModel)?.getListeners(TableLoadingDecorator::class.java)?.isNotEmpty() == true) {
        null
      }
      else {
