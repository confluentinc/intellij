...
class SshTunnelComponent<T>(val project: Project,
  ...
  val isEnabledCheckBox = CheckBox(KafkaMessagesBundle.message("settings.tunnel.is.enabled") + additionalLabel).apply {
    addActionListener {
      updateIsEnabled()
    }
  }
    ...
    indent {
      ...
      if (showLocalPort) {
        row = row(KafkaMessagesBundle.message("settings.tunnel.ssh.local.port")) {
          ...
          localPort?.component?.emptyText?.text = KafkaMessagesBundle.message("settings.tunnel.ssh.local.port.empty.text")
        }
      }
  ...
  private fun installValidatorOnCurrentSshComboBox() {
    val sshConfigValidatorSupplier = Supplier {
      ...
      when {
        ...
        selectedConfig == null -> ValidationInfo(KafkaMessagesBundle.message("settings.validator.ssh.config.not.selected"), sshComboBox.comboBox)
    ...
    val sshConfigValidator = ComponentValidator(sshComboBox).withValidator(sshConfigValidatorSupplier).withFocusValidator(
      sshConfigValidatorSupplier).installOn(sshComboBox.comboBox)
  ...
  override fun getValue(): ConnectionSshTunnelData {
    ...
    return ConnectionSshTunnelData(isEnabledCheckBox.isSelected, configId, localPort = localPort?.component?.text?.toIntOrNull())
  }
