...
object RfsNotificationUtils {
  ...
  private fun notifySuccess(@NotificationContent message: String, @NotificationTitle title: String, actions: List<DumbAwareAction>,
                            project: Project? = null) = invokeLater {
    val notification = Notification(ACTION_FAILURE_NOTIFICATION_GROUP,
                                    title,
                                    message,
  ...
  fun notifySuccess(@NotificationContent message: String,
                    @NotificationTitle title: String,
                    action: DumbAwareAction? = null,
                    project: Project? = null) = notifySuccess(message, title, listOfNotNull(action), project)

  // ToDo This looks like a copy of TestConnectionPanelWrapper.showDetailsPopup, we should extract common part.
  fun showExceptionMessage(project: Project?,
                           e: Throwable,
                           @DialogTitle title: String = KafkaMessagesBundle.message(
                             "notification.title.remote.fs.error")): Unit = invokeAndWaitIfNeeded {
    ...
    val stackScrollPane = ScrollPaneFactory.createScrollPane(stackEditor, true).apply {
      preferredSize = Dimension(bestWidth, min(JBUIScale.scale(200), max(2 * lineHeight, stackEditor.preferredSize.height)))
    }

    val stackPanel = HideableTitledPanel(KafkaMessagesBundle.message("testConnection.stacktrace"), true, stackScrollPane, false).apply {
  ...
  fun notifyException(e: Throwable, @NotificationTitle title: String? = null, @DialogTitle detailsTitle: String? = null) {
    logger.info("RFS Error", e)
    notify(title ?: KafkaMessagesBundle.message("notification.title.remote.fs.error"), e.toPresentableText(), getStackTrace(e), detailsTitle)
  }

  private fun showDetailsDialog(@DialogTitle detailsTitle: String?, @NotificationContent message: String, stacktrace: String) {
    ...
    val detailsConsole = ConsoleViewImpl(project, true).apply {
      ...
      preferredSize = Dimension(ScreenUtil.getMainScreenBounds().width / 4, min(ScreenUtil.getMainScreenBounds().height / 3,
  ...
  private fun notify(@NotificationTitle title: String,
